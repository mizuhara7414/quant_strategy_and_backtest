...
HMM 多標的輪動策略（實盤推論版）
─────────────────────────────────────────────────────────
與 hmm_rotation.py 的差異：
  使用 Forward Algorithm 逐日推論，每個時間點只使用
  截至當天為止的資料，不洩漏未來資訊，更接近實盤行為。
  原版 model.predict() 使用全序列維特比演算法（開上帝視角）。
─────────────────────────────────────────────────────────
HMM 用台股特徵判斷市場狀態：
...
# ── Forward Algorithm 即時推論 ───────────────────────────────────────────────
def forward_decode(model: GaussianHMM, X: np.ndarray) -> np.ndarray:
    """
    用 Forward Algorithm 逐日推論隱藏狀態。
    每個時間點只使用截至當天的資料，不洩漏未來資訊。

    對比 model.predict()（維特比演算法）：
      - 維特比：用全序列找最優路徑，等於開上帝視角
      - Forward：只看到當天為止，取當下後驗機率最高的狀態
    """
    n, n_states = len(X), model.n_components

    # 預先計算所有時間點的對數發射機率
    log_emit = np.zeros((n, n_states))
    for s in range(n_states):
        log_emit[:, s] = multivariate_normal.logpdf(
    ...
    # Forward pass：α_t(s) = P(觀測到 x_1..x_t 且第 t 天狀態為 s)
    log_alpha = np.zeros((n, n_states))
    log_alpha[0] = log_startprob + log_emit[0]

    for t in range(1, n):
        for s in range(n_states):
            log_alpha[t, s] = (logsumexp(log_alpha[t-1] + log_transmat[:, s])
                               + log_emit[t, s])
...
def main():
    ...
    print(f"\n即時推論（Forward Algorithm，不使用未來資料）...")
